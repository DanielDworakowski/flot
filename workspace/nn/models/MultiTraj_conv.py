...
class Resnet_MultiConv(nn.Module):

    def __init__(self, nOuts):
        super(Resnet_MultiConv, self).__init__()
        #
        ...
        last = list(self.base.children())[-3:-1] # Copy the last layers.
        modules = list(self.base.children())[:-3] # delete the last fc layer.
        ...
        for i in range(self.rangeY):
            for j in range(self.rangeX):
                subnet = copy.deepcopy(nn.Sequential(*last))
                fc = nn.Linear(numFeat, 2)
                self.subnet.append(subnet) # Binary classifier.
                self.fc.append(fc)
                self.add_module('subnet%s'%len(self.subnet), subnet)
    ...
    def forward(self, x):
        ...
        for row in range(self.rangeY):
            for col in range(self.rangeX):
                ...
                subnetOut = self.subnet[idx](feat)
                out.append(self.fc[idx](subnetOut.view(subnetOut.size(0), -1)))
